...
Though no one really agrees on its precise definition, object oriented programming has shaped the design of many programming languages. The core idea in object oriented programming is to divide programs into smaller pieces and make each piece responsible for managing it's own state. This way, some knowledge about the way a piece of the program works be kept local to that piece.
##
